...
from 检查判断 import checksuanshi  # 导入 checksuanshi 函数
import re  # 导入正则表达式模块
from 四则运算实现 import *  # 导入 calculate 模块中的所有内容
from 数值范围 import random_parameter  # 导入 random_parameter 函数
...
def checkrepeat(q_list, all_question, a_list, all_answer):
    if (len(all_question) and len(all_answer)):
        ...
        symbol = []
        all_symbol = []
        for alnum in pro_suanshi(q_list[0]):
            if alnum in const_op:
                symbol.append(alnum)

        numbernum=[x for x in pro_suanshi(q_list[0]) if x not in symbol]
#
        for alqust in all_question:
            num = 0
            all_num = []
            for alqustnum in pro_suanshi(alqust):
            ...
            all_num=[x for x in pro_suanshi(alqust) if x not in all_symbol]
...
def Rmain(maxNum, formula_count):
    # 存储算式
    formula_list = []
    # 存储答案
    answer_list = []
    ...
    while (count <= formula_count):
        ...
        answer = checksuanshi(formula, maxNum)
        # 判断是否重复
